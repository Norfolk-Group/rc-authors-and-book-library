...
import { Brain, Search, BookOpen, UserCircle, FileText, Video, Headphones, Mic, X, ArrowLeft, Sparkles, Globe, ExternalLink, Quote } from "lucide-react";
...
export default function ThematicSearch() {
  ...
  // Opt-in web augmentation (Exa + Perplexity) — only fires when the user asks,
  // to avoid surprise paid-API calls on every library search.
  const { data: webData, isFetching: webFetching } = trpc.webSearch.research.useQuery(
    { query: activeQuery, numResults: 6 },
    { enabled: webEnabled && activeQuery.length >= 2, staleTime: 5 * 60 * 1000, retry: 1 }
  );
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        {!isLoading && !isFetching && activeQuery && (
          <>
            ...
            {/* Opt-in: blend live web results (Exa + Perplexity) into library search */}
            <div className="pt-2">
              {!webEnabled ? (
                <button
                  onClick={() => setWebEnabled(true)}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-border/70 text-sm text-muted-foreground hover:text-foreground hover:bg-accent/30 transition-colors"
                >
                  <Globe size={15} />
                  Also search the web for “{activeQuery}”
                </button>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Globe size={15} className="text-primary" />
                    <h2 className="text-sm font-semibold">From the web</h2>
                  ...
                  {!webFetching && webData?.answer && (
                    <div className="rounded-xl border border-border/60 bg-card p-4 space-y-2">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                        <Quote size={13} /> Answer
                      </div>
                      <p className="text-sm leading-relaxed whitespace-pre-wrap text-card-foreground">{webData.answer}</p>
                    </div>
                  )}
                  {!webFetching && (webData?.sources?.length ?? 0) > 0 && (
                    <div className="space-y-2">
                      {webData!.sources.map((s, i) => (
                        <a
                          ...
                          className="group flex items-start gap-3 px-4 py-3 rounded-xl border border-border/60 bg-card hover:bg-accent/30 transition-all"
                        >
                          <Globe size={15} className="text-muted-foreground flex-shrink-0 mt-0.5" />
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold leading-tight line-clamp-2 text-card-foreground group-hover:text-primary transition-colors">{s.title}</p>
                            {s.snippet && <p className="text-xs text-muted-foreground/80 line-clamp-2 mt-0.5">{s.snippet}</p>}
                          </div>
                  ...
                  {!webFetching && !webData?.answer && (webData?.sources?.length ?? 0) === 0 && (
                    <p className="text-xs text-muted-foreground italic">
                      No web results — check that EXA_API_KEY / Perplexity are configured.
                    </p>
                  )}
                </div>
              )}
